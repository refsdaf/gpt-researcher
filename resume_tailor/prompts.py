...
TAILOR_SUMMARY_PROMPT = """You are a professional resume writer. Rewrite the Professional Summary to match the Job Description.
...
**Instructions:**
- Formula: [Adjective from JD] [Current Title/Target Title] with [Number] years of experience. Expert in [Skill 1] and [Skill 2]. Proven track record of [Major Achievement relevant to JD].
...
4. **Irrelevant Info**: If `remove_irrelevant` is True, remove bullets that have NO connection to the new role (e.g., "office party planning" for a coding role).
5. **Exaggeration Level**:
   - If `exaggerate` is False: Stick strictly to the facts but frame them positively.
   - If `exaggerate` is True: Use "Power Verbs" and slightly aggressive framing of impact, but do not lie about skills you don't possess.
...
TAILOR_SKILLS_PROMPT = """You are optimizing the "Skills" section for an ATS (Applicant Tracking System).
...
**Instructions:**
1. **Exact Phrasing**: Use the exact specific skill names found in the JD (e.g., use "MS Excel" instead of "Microsoft Office" if the JD says so).
2. **Categorization**: Group skills logically (e.g., "Languages", "Tools", "Frameworks") matching the JD's structure if apparent.
3. Prioritize skills mentioned in the JD.

Return the result as a list of strings or a dictionary of categories, whichever matches the input structure best.
"""
